...
export default function List(): JSX.Element {
  ...
  const showExchange = ([exchange]: PageProps["data"]["0"]): boolean =>
    store.currentExchange.has(exchange);

  const transducerFilter = createMemo(() =>
    store.originalData.reduce(
      pipe(TFilter(showExchange), TFilter(showSearch))(TArrayConcat),
      []
    )
  );
  ...
  const handleExchangeChange = (exchange: string) => {
    return (e) => {
      ...
      store.currentExchange._data.forEach((exchange) =>
      ...
      <div class="max-w-full mx-auto md:max-w-3xl">
        ...
        <div class="flex mb-4 gap-2">
          ...
          <Show
            when={
              store.search ||
              store.currentExchange._data.length <
                Object.keys(store.metadata).length
            }
        ...
        <Show
          ...
          fallback={
            <div class="flex justify-center text-xl">
              Please search or select new filter
            </div>
          }
        >
          <ShariahList
            data={transducerFilter()}
            exchangeStyle={store.exchangeStyle}
          />
        </Show>
      </div>
...
const ShariahList = (props: {
  data: PageProps["data"];
  exchangeStyle: Record<string, string>;
}) => {
  return (
    <ul
      class="flex flex-col overflow-auto gap-1 overscroll-contain"
    ...
    >
      <For each={props.data}>
        {(item) => (
          <ShariahListItem
            item={item}
            exchangeColor={props.exchangeStyle[item[0]]}
          />
        )}
      </For>
    </ul>
...
const ShariahListItem = (props: {
  item: PageProps["data"]["0"];
  ...
  return (
    <li class="flex items-center group gap-x-1" role="listitem">
      ...
      <a
        target="_blank"
        href={`https://www.tradingview.com/symbols/${exchange}-${code}`}
      >
