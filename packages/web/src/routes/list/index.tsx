...
import { createMemo, createResource, For, JSX, onMount, Show, Suspense } from 'solid-js'
...
export function routeData({ location }) {
  return createResource<PageProps | undefined>(
    async () => {
      ...
      const search = new URLSearchParams(location.search) // { exchange: ['MYX', 'NASDAQ', 'SSE'], q: "search key" }

      const queryParams = {
        q: search.get('q') ?? '',
        exchange: search.getAll('exchange') ?? [],
      }
      ...
      return {
        ...
        metadata: m,
        queryParams,
        exchangesList: queryParams.exchange.length === 0 ? Object.keys(metadata) : queryParams.exchange,
      }
    },
    ...
    if (!data.loading) {
      ...
      setStore(
        ...
        Object.keys(store.metadata).reduce(
          (acc, key, i) => ({ ...acc, [key]: staticExchangeColors[i % staticExchangeColors.length] }),
          {}
        )
      )
    }
        ...
        <Show when={data()} keyed>
          <div class='mx-auto max-w-3xl py-10 px-5'>
            ...
            <Show
              ...
              fallback={
                <div class='flex justify-center text-xl text-gray-500'>Please search or select new filter</div>
              }>
