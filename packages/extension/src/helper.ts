...
export async function setStockListInMap(): Promise<void> {
  try {
    ...
    console.warn(
      `Tradingview Shariah Indicator: Please refresh the browser, Error:`,
      e
    );
  }
}

export function getStockStat(
  symbolWithExchange: `${string}:${string}`,
  defaultReturn = {}
): Record<string, number> {
  return SHARIAH_LIST.has(symbolWithExchange)
    ? SHARIAH_LIST.get(symbolWithExchange)
    : defaultReturn;
}
...
export function deleteShariahIcon(
  parentElement: Element | Document = document
): void {
  parentElement
    .querySelectorAll(`[${attributeName}="${extensionName}"]`)
...
export async function setStorage<K extends keyof TSI.StorageMap>(
  key: K,
  payload: TSI.StorageMap[K]
): Promise<void> {
  try {
...
export async function getStorage<K extends keyof TSI.StorageMap>(
  key: K
): Promise<TSI.StorageMap[K]> {
  try {
