...
export async function setStockListInMap() {
  let MYX = {}
...
export function getStockStat(symbol, defaultReturn = { s: 0 }) {
  return SHARIAH_LIST.has(symbol) ? SHARIAH_LIST.get(symbol) : defaultReturn
}

export function isSyariahIconExist(element) {
  return element.querySelector(`[${attributeName}="${extensionName}"]`)
}

export function deleteSyariahIcon(parentElement = document) {
 ...
 */
export function createIcon({ width, height } = { width: 25, height: 25 }) {
...
export function observeNodeChanges(nodeToObserve, cb, options = { childList: true, subtree: true }) {
  let observer
