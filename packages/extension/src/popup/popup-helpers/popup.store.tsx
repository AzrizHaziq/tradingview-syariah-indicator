...
type CurrentDataStore = [
  Accessor<TSI.Flag[]>,
  { setState?: (item: TSI.Flag[]) => void }
];

const CurrentDataContext = createContext<CurrentDataStore>([() => [], {}]);

export const useCurrentData = (): CurrentDataStore =>
  useContext(CurrentDataContext);

export const CurrentDataProvider: Component<{ value: TSI.Flag[] }> = (
  props
) => {
  ...
  return (
    <CurrentDataContext.Provider value={store}>
      {props.children}
  ...
  try {
    ...
    if (process.env.NODE_ENV !== "production") {
      return DETAILS.map((i) => ({
        ...i,
        updatedAt: `${Math.floor(Math.random() * 12)}/${Math.floor(
          Math.random() * 25
        )}/2021`,
      }));
    }
