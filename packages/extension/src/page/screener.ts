...
const shariah = {
  ...
  createIcon: () => {
    ...
    icon.setAttribute(
      `${attributeName}-filter-icon`,
      `${extensionName}-filter-icon`
    );
  ...
  onClick:
    ...
    async (e: Event & { target: { checked: boolean } }) => {
      try {
        ...
        wrapperElement.setAttribute(
          "title",
          shariah.status[`${shariah.currentState}`]
        );
      } catch (e) {
...
function observedTableChanges() {
  ...
  const tableNode = document.querySelector(
    ".tv-screener__content-pane table tbody.tv-data-table__tbody"
  );
    ...
    Array.from(tableNode.children).forEach((tr) => {
      ...
      if (isSyariah) {
        ...
        if (isShariahIconExist(firstColumn)) {
          ...
          const domToBeAdded = firstColumn.querySelector(
            ".tv-screener__symbol"
          );
...
function observedCountryFlagChanges() {
  ...
  const countryMarketDropdown = document.querySelector(
...
function setupFilterBtn(state) {
  ...
  if (
    document.querySelector(
      `label[${state.checkBoxAttrName}=${state.checkBoxAttrValue}]`
    )
  ) {
    return;
  }
  ...
  wrapper.className = document.querySelector(
    ".tv-screener-toolbar__button--refresh"
  ).className; // copy refresh btn class
  ...
  wrapper.addEventListener("change", async function (e) {
    try {
      ...
      state.currentState
        ? tbody.classList.add(state.css.body)
        : tbody.classList.remove(state.css.body);
    } catch (error) {
...
function setupCssClassName() {
  document
    ...
    .classList.add(
      ...[css.main.body, shariah.currentState ? css.shariah.body : ""].filter(
        Boolean
      )
    );
}
...
function isCountryValid(): boolean {
  ...
  return ONLY_VALID_COUNTRIES.some((country) =>
    document.querySelector(
      `.tv-screener-market-select__button > img.tv-flag-country.tv-flag-country--${country}`
    )
  );
}
