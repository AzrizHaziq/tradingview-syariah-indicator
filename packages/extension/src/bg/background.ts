  ...
  debounce(
    async function listener(_, { status }, { url }): Promise<void> {
      ...
      try {
        ...
        const shouldUseCacheValue =
          differenceInDays(new Date(), lastFetchAt) >= 0;
...
async function setListInStorages(
  response: TSI.RESPONSE_FROM_JSON
): Promise<void> {
  try {
    const allExchanges = Object.entries(response).flatMap(
      ([exchange, exchangeDetail]) => {
        ...
        return Object.entries(list).map(([symbol, symbolData]) => {
          const val = symbolData.reduce(
            (acc, value, index) => ({
              ...acc,
              [shape[index].hasOwnProperty(value)
                ? shape[index][value]
                : shape[index].default]: value,
            }),
...
async function setExchangeDetailInfoInStorage(
  response: TSI.RESPONSE_FROM_JSON
): Promise<void> {
  try {
    const exchangesDetails: TSI.Flag[] = Object.entries(response).map(
      ([exchange, { updatedAt, list }]) => ({
        id: exchange,
        counts: Object.keys(list).length,
        updatedAt: isDate(new Date(updatedAt))
          ? format(new Date(updatedAt), "dd LLL yy")
          : "--",
      })
    );
